...
from data1.serialize import serialize_arr, deserialize_str, SerializerSettings
...
class Scaler:
    """
    Represents a data1 scaler with transformation and inverse transformation functions.
...
def get_scaler(history, alpha=0.95, beta=0.3, basic=False):
    """
    Generate a Scaler object based on given history data1.
...
def generate_predictions(
    ...
    **kwargs
    ...
    completions_list = []
    complete = lambda x: completion_fn(input_str=x, steps=steps*STEP_MULTIPLIER, settings=settings, num_samples=num_samples, temp=temp, whether_blanket=whether_blanket, genai_key=genai_key)
    # 位置
...
def get_llmtime_predictions_data(train, test, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, whether_blanket=True, genai_key=None, **kwargs):
    """
    ...
    Args:
        train (array-like or list of array-like): Training time series data1 (history).
        test (array-like or list of array-like): Test time series data1 (true future).
        ...
        basic (bool, optional): If True, use the basic version of data1 scaling. Defaults to False.
    ...
    """
    # 这里不是直接预测，还必须在原代码里有阐述(有趣, 可以说是形成了体系)
    ...
    if isinstance(settings, dict):
        settings = SerializerSettings(**settings)  # 将其字典化
    ...
    # 此处 input_str 是tuple, 每个元素是一个数据集
    ...
    if num_samples > 0:
        preds, completions_list, input_strs = generate_predictions(completion_fn, input_strs, steps, settings, scalers,
                                                                    ...
                                                                    parallel=parallel, whether_blanket=whether_blanket, genai_key=genai_key, **kwargs)
        # 位置
